...
def get_feature_flow(label_pcap, select_packet_len, packets_num, start_index=76, add_sep=True):
    ...
    # [MODIFIED] Logic changed from < 3 to < 1 to keep all flows
    if len(feature_result) == 0:
        ...
        # [MODIFIED]
        if len(feature_result[label_pcap, extract_keys[1], extract_keys[2]].ip_lengths) < 1:
            print("preprocess udp flow %s but this flow has less than 1 packets." % label_pcap)
            return -1
    # [MODIFIED]
    elif len(packets) < 1:
        print("preprocess tcp flow %s but this flow has less than 1 packets." % label_pcap)
        return -1
    try:
        # [MODIFIED]
        if len(feature_result[label_pcap, 'tcp', '0'].ip_lengths) < 1:
            print("1: preprocess flow %s but this flow has less than 1 packets." % label_pcap)
            return -1
    except Exception as e:
        #print("*** this flow begins from 1 or other numbers than 0.")
        for key in feature_result.keys():
            # [MODIFIED]
            if len(feature_result[key].ip_lengths) < 1:
                print("2: preprocess flow %s but this flow has less than 1 packets." % label_pcap)
                return -1
...
def convert_splitcap(pcapng_path, pcap_path,pcap_split_path,is_pcap_label=False):
        ...
        for dir in dd:
            for _,_,ff in os.walk(pcap_split_path+"splitcap" + "/" + dir):
                for f in ff:
                    file_size = float(size_format(os.path.getsize(pcap_split_path+"splitcap" + "/" + dir + "/" + f)))
                    # 2KB
                    # [MODIFIED] Commented out to prevent deleting small flows
    ...
    for p,dd,ff in os.walk(pcap_split_path+"splitcap"):
        for dir in dd:
            for _,_,ff in os.walk(pcap_split_path+"splitcap" + "/" + dir):
                print(dir,len(ff))
                # [MODIFIED] Commented out to prevent deleting small classes
                # if len(ff)<10:
...
# ============================================================================
# CSTNET Support Functions (Added)
# ============================================================================

def parse_ipv4_frames_hex(hex_string):
    """
    Parses a long hex string containing multiple IPv4 frames into individual packet strings.
    """
    hex_string = hex_string.replace(" ", "").replace("\n", "").lower()
    if not hex_string:
        return []
    
    # Simple recursive parsing based on IPv4 Total Length field
    frames = []
    current_idx = 0
    total_len_chars = len(hex_string)
    
    while current_idx < total_len_chars:
        # Check for IPv4 version (4) and IHL (usually 5) -> '45'
        # This is a basic check.
        if hex_string[current_idx] != '4':
            # Skip invalid or padding
            current_idx += 1
            continue
            
        # Total Length is at offset 2 bytes (4 hex chars)
        # IP header structure: [VerIHL][TOS][TotalLen]...
        # Indices: 0,1 (VerIHL), 2,3 (TOS), 4,5,6,7 (TotalLen)
...
def process_cstnet_row(hex_string, payload_length=64, payload_packet=5, start_index=76):
    """
    Transforms a CSTNET hex string (concatenated IP packets) into TrafficFormer input format.
    Mimics get_feature_flow logic for "No_ether" case.
    """
    packets = parse_ipv4_frames_hex(hex_string)
    if not packets:
        return None
    ...
    # Determine "Client" IP from first packet to assign direction
    # IP Source Address is at offset 12 bytes (24 hex chars)
    client_ip = None
    if len(packets[0]) >= 32:
    ...
    for packet_string in packets:
        ...
        # Add Fake Ethernet Header (TrafficFormer logic)
        # Forward: c49a..., Backward: e46f...
        # 28 hex chars = 14 bytes
        if is_forward:
            packet_string = "c49a025996f8e46f13e2e3ae0800" + packet_string
        else:
            packet_string = "e46f13e2e3aec49a025996f80800" + packet_string
            
        # Truncate to payload_length
        # start_index is char index. TrafficFormer default 76.
        if len(packet_string) > start_index:
            packet_string = packet_string[start_index : start_index + 2*payload_length]
        else:
            # Too short, take what we have or skip? 
            # Usually strip header logic implies taking payload.
            # If string is shorter than start_index, it's empty payload.
            packet_string = ""
        ...
        # Bigram Generation
        # Assuming bigram_generation takes a clean hex string
        flow_data_string += bigram_generation(packet_string.strip(), token_len=len(packet_string.strip()), flag=True)
    ...
    return flow_data_string.strip()

def generate_dataset_from_cstnet(dataset_path, output_path):
    """
    Generates TrafficFormer TSV files from CSTNET .npy files.
    """
    print(f"Processing CSTNET data from {dataset_path}...")
    ...
    for split in splits:
        # Handle filename variations (val vs valid)
        npy_name = f"x_datagram_{split}.npy"
        label_name = f"y_{split}.npy"
        
        if not os.path.exists(os.path.join(dataset_path, npy_name)):
            if split == 'valid' and os.path.exists(os.path.join(dataset_path, "x_datagram_val.npy")):
                npy_name = "x_datagram_val.npy"
                label_name = "y_val.npy"
            else:
        ...
        try:
            x_data = np.load(os.path.join(dataset_path, npy_name), allow_pickle=True)
            y_data = np.load(os.path.join(dataset_path, label_name), allow_pickle=True)
        except Exception as e:
        ...
        # Process each flow
        for i in tqdm(range(len(x_data)), desc=f"Generating {split}"):
            hex_str = x_data[i]
            label = y_data[i]
            
            # Use default TrafficFormer params (payload=64, packets=5) or adjustable?
            # Using defaults from the script logic: 64 bytes, 5 packets, start_index 76
            feature_str = process_cstnet_row(hex_str, payload_length=64, payload_packet=5, start_index=76)
            
            if feature_str:
                processed_data.append(feature_str)
                processed_labels.append(label)
                
        # Write TSV
        out_name = f"{split}_dataset.tsv"
        if split == 'val': out_name = "valid_dataset.tsv" # Consistency
        
        write_dataset_tsv(processed_data, processed_labels, output_path + "/", split if split != 'val' else 'valid')
        print(f"Saved {out_name} with {len(processed_data)} samples.")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--cstnet_path", type=str, help="Path to CSTNET .npy directory")
    parser.add_argument("--output_path", type=str, default="./datasets/cstnet", help="Output directory")
    args = parser.parse_args()
    
    if args.cstnet_path:
        generate_dataset_from_cstnet(args.cstnet_path, args.output_path)
    else:
        print("Please provide --cstnet_path to process CSTNET data.")