...
#     # Calculate metrics
#     # macro returns single floats (scalars) when average='weighted'
#     macro = precision_recall_fscore_support(target_all, pred_all, average='weighted')
#     cm = confusion_matrix(target_all, pred_all)

#     # compute acc, precision, recall, f1 for each class
#     # average=None returns numpy arrays
#     acc = accuracy_score(target_all, pred_all)
#     pre_per_class, rec_per_class, f1_per_class, support_per_class = precision_recall_fscore_support(target_all, pred_all, average=None)
...
#     # --- 修改部分开始 ---
#     # 使用 float() 强制转换标量，解决 'float' object has no attribute 'tolist' 报错
#     test_state['weighted_pre'] = float(macro[0])
#     test_state['weighted_rec'] = float(macro[1])
#     test_state['weighted_f1'] = float(macro[2])
#     test_state['acc'] = float(acc)
    
#     # cm 是 numpy 矩阵，使用 tolist() 转换为 list
#     test_state['cm'] = cm.tolist()
    
#     # 下面这些是 numpy 数组，建议也加上 .tolist() 以便后续 JSON 存储，防止 TypeError
#     test_state['pre_per_class'] = pre_per_class.tolist()
#     test_state['rec_per_class'] = rec_per_class.tolist()
#     test_state['f1_per_class'] = f1_per_class.tolist()
#     test_state['support_per_class'] = support_per_class.tolist()
#     # --- 修改部分结束 ---
...
#     # 改动1: 这里的列表建议先存 Tensor (在 GPU 上)，方便后面做 all_gather
#     pred_list = []
#     target_list = []
...
#         # 改动2: 不要在这里转 .cpu()，保持在 GPU 上以便进行多卡通信
#         # pred[0] shape: [batch_size], target shape: [batch_size]
...
#     # --- 核心修复开始: 多卡数据收集 (All-Gather) ---
    
#     # 1. 先把当前卡的所有 batch 拼成一个长 Tensor
#     pred_local = torch.cat(pred_list)
#     target_local = torch.cat(target_list)
...
#         # 创建列表来接收所有卡的数据
#         # 需要确保所有 tensor 都在同一个维度，这里一般没问题
#         preds_gathered = [torch.zeros_like(pred_local) for _ in range(world_size)]
#         targets_gathered = [torch.zeros_like(target_local) for _ in range(world_size)]
        
#         # 执行 all_gather
#         dist.all_gather(preds_gathered, pred_local)
#         dist.all_gather(targets_gathered, target_local)
        
#         # 拼接所有卡的数据
#         pred_final = torch.cat(preds_gathered)
#         target_final = torch.cat(targets_gathered)
#     else:
#         # 单卡环境直接使用
#         pred_final = pred_local
#         target_final = target_local

#     # 3. 转回 CPU 给 sklearn 使用
#     pred_all_np = pred_final.cpu().numpy()
#     target_all_np = target_final.cpu().numpy()
    
#     # --- 核心修复结束 ---

#     # Calculate metrics using gathered data
#     weighted = precision_recall_fscore_support(target_all_np, pred_all_np, average='weighted')
#     macro = precision_recall_fscore_support(target_all_np, pred_all_np, average='macro')
#     cm = confusion_matrix(target_all_np, pred_all_np)

#     acc = accuracy_score(target_all_np, pred_all_np)
#     pre_per_class, rec_per_class, f1_per_class, support_per_class = precision_recall_fscore_support(target_all_np, pred_all_np, average=None)

#     # gather the stats from all processes (MetricLogger 负责 Loss/Acc 的同步)
#     metric_logger.synchronize_between_processes()
...
#     # 修复 tolist() 报错问题 + 使用全局数据计算的结果
#     test_state['weighted_pre'] = float(weighted[0])
#     test_state['weighted_rec'] = float(weighted[1])
#     test_state['weighted_f1'] = float(weighted[2])
#     test_state['macro_pre'] = float(macro[0])
#     test_state['macro_rec'] = float(macro[1])
#     test_state['macro_f1'] = float(macro[2])
#     test_state['acc'] = float(acc)
#     test_state['cm'] = cm.tolist()
...
def evaluate(data_loader, model, device, if_stat=False):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        # [修复 2] 动态判断类别数，防止类别<5时报错
        # output.shape[1] 是当前模型的类别数量
        batch_size = images.shape[0]
        ...
        if num_classes >= 5:
            ...
            # 如果类别少于5，只计算 Top-1，Top-5 设为 0 以防报错
