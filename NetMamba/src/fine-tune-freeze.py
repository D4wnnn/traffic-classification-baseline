...
def get_args_parser():
    parser = argparse.ArgumentParser('NetMamba fine-tuning (Freeze Encoder) for traffic classification', add_help=False)
    # 64
    parser.add_argument('--batch_size', default=64, type=int,
                        help='Batch size per GPU (effective batch size is batch_size * accum_iter * # gpus')
    parser.add_argument('--epochs', default=50, type=int, help='Linear probe usually needs fewer epochs or converges differently') 
    ...
    # Layer decay is removed/ignored in logic, but kept in args to prevent crash if passed
    parser.add_argument('--layer_decay', type=float, default=0.75,
                        help='layer-wise lr decay (IGNORED in freeze mode)')
    ...
    parser.add_argument('--output_dir', default='./output/finetune_freeze',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./output/finetune_freeze',
...
def main(args):
    ...
    # 2. Freeze Logic (IMPORTANT)
    print("=============================================================")
    print("                FREEZING ENCODER (BACKBONE)                  ")
    print("=============================================================")
    for name, param in model.named_parameters():
        if "head" not in name: # Lock everything except the head
    ...
    print('Total params (M): %.2f' % (total_parameters / 1.e6))
    print('Trainable params (M): %.2f (Should be small, head only)' % (n_parameters / 1.e6))
    ...
    # 3. Optimizer for Frozen training
    # We do NOT use layer-wise decay for linear probing/frozen backbone.
    # We simply filter for requires_grad=True
    ...
    best_checkpoint = torch.load(os.path.join(args.output_dir, "checkpoint-best.pth"), map_location='cpu', weights_only=False)
    ...
    with open(os.path.join(args.output_dir, "test_stats.json"), mode="w", encoding="utf-8") as f:
        json.dump({
            ...
            "macro_pre": test_result["macro_pre"],
            "macro_rec": test_result["macro_rec"],
