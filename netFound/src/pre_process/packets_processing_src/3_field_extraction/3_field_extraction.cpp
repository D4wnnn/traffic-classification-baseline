...
int main(int argc, char *argv[]) {
    if (argc < 3 || argc > 4) {
        std::cout << "Usage: " << argv[0] << " <input path> <output path> [tcpoptions_flag]" << std::endl;
        return 1;
    }
    ...
    if (std::filesystem::is_directory(input_path)) {
        ...
        for (const auto &entry : std::filesystem::directory_iterator(input_path)) {
            if (entry.is_regular_file()) {
                std::string input_file = entry.path().string();
                std::string output_file = output_path_str + "/" + entry.path().filename().string();
                process_file(input_file, output_file, tcpoptions_flag);
            }
        }
    } else if (std::filesystem::is_regular_file(input_path)) {
        // 新逻辑：如果是单个文件，直接处理
        // 注意：这种模式下，output_path_str 就是完整的目标文件路径前缀
        process_file(input_path_str, output_path_str, tcpoptions_flag);
    } else {
        std::cerr << "Error: Input path '" << input_path_str << "' is neither a directory nor a regular file." << std::endl;
        return 1;
    }
