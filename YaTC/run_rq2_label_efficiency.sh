...
DATA_BASE_DIR="${DATA_BASE_DIR:-./1-data_processing/outputs}"
...
PRETRAINED_MODEL_PATH="${PRETRAINED_MODEL_PATH:-./YaTC_pretrained_model.pth}"
EPOCHS="${EPOCHS:-20}"
...
for spec in $DATASETS; do
  ...
  for ratio in $RATIOS; do
    ...
    rm -rf "$output_dir"
    mkdir -p "$log_dir"
    echo "[RQ2][YaTC] dataset=$dataset ratio=$ratio classes=$nb_classes output=$output_dir"
    ...
    "$PYTHON_BIN" final-test.py \
      --data_path "$rq2_data_dir" \
      --checkpoint "$checkpoint" \
      --model TraFormer_YaTC \
      --nb_classes "$nb_classes" \
      --batch_size "$BATCH_SIZE" \
      --seed "$SEED" 2>&1 | tee "$output_dir/test.log"
  done
done
