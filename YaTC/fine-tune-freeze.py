...
def get_args_parser():
    ...
    parser.add_argument('--freeze_encoder', action='store_true', help='Freeze the backbone/encoder parameters')
    return parser
...
def main(args):
    ...
    if args.freeze_encoder:
        print("---------------------------------------------")
        print("Freezing Packet Encoder (Backbone)...")
        for name, param in model.named_parameters():
            if 'head' not in name:  # 如果参数名不包含 'head'，则冻结
                param.requires_grad = False
            else:
                print(f"Training parameter: {name}")  # 打印出参与训练的参数（应该是 head.weight 和 head.bias）
        print("---------------------------------------------")
    # ================= 插入结束 =================
    model.to(device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if test_stats["acc1"] > max_accuracy:
            print(f"New best accuracy: {test_stats['acc1']:.4f} (was {max_accuracy:.4f}). Saving checkpoint-best.pth ...")
            if args.output_dir:
                misc.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                    loss_scaler=loss_scaler, epoch=epoch, name='best'
                )
